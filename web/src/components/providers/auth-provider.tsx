...
export function AuthProvider({
    ...
    try {
      ...
      if (res.ok && data.data?.accessToken) {
        ...
        if (pathname.startsWith("/dashboard") || pathname.startsWith("/runs") ||
            pathname.startsWith("/approvals") || pathname.startsWith("/tools") ||
            pathname.startsWith("/audit") || pathname.startsWith("/settings")) {
          router.push("/login");
        }
      }
    } catch {
  ...
  // Periodic refresh before 15m token expiry (every 10m)
  useEffect(() => {
