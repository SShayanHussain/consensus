 ...
 * Used in API routes to enforce tenant isolation (PLAYBOOK §4).
 */
 ...
 * Used for administrative actions (billing, tool policy, member management).
 */
...
/**
 * Ensures the user is at least a reviewer in the given workspace.
 * Reviewers can approve/reject runs but cannot change tool policy.
 */
export async function requireReviewer(workspaceId: string): Promise<Session | Response> {
  const session = await requireAuth();
  if (session instanceof Response) return session;
  ...
  if (!membership || !["owner", "reviewer"].includes(membership.role)) {
    return errorResponse("FORBIDDEN", "Only owners and reviewers can perform this action", 403);
  }
